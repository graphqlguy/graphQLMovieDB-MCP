# graphQLMovieDB-MCP - Class 16

This branch is the repository state at the end of [Class 16: Agent-Specific Security](https://graphqlguy.com/docs/tutorial-graphql-mcp/agent-specific-security), part of the [GraphQL + MCP course](https://graphqlguy.com/docs/tutorial-graphql-mcp/mcp-and-graphql).

**What Class 16 adds:** `SafeMutationTools` for stage-and-confirm writes, `AuditAspect` for the audit trail, and `SessionRateLimitInterceptor` for per-session limits.

**Following along:** start from `mcp_class_15`, work through the lesson, then compare your result with this branch. The next class continues on `mcp_class_17`.
