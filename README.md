# graphQLMovieDB-MCP - Class 8

This branch is the repository state at the end of [Class 8: Tool Descriptions From the Schema](https://graphqlguy.com/docs/tutorial-graphql-mcp/tool-descriptions-from-schema), part of the [GraphQL + MCP course](https://graphqlguy.com/docs/tutorial-graphql-mcp/mcp-and-graphql).

**What Class 8 adds:** `SchemaDescriptionProvider` and `SchemaDescriptionToolSpecifications`, so a tool's description comes from the schema description instead of a second copy in Java.

**Following along:** start from `mcp_class_7`, work through the lesson, then compare your result with this branch. The next class continues on `mcp_class_9`.
