# graphQLMovieDB-MCP - Class 4

This branch is the repository state at the end of [Class 4: Apollo MCP Server in Twenty Minutes](https://graphqlguy.com/docs/tutorial-graphql-mcp/apollo-mcp-server-in-twenty-minutes), part of the [GraphQL + MCP course](https://graphqlguy.com/docs/tutorial-graphql-mcp/mcp-and-graphql).

**What Class 4 adds:** the `apollo/` directory: a schema copy and the two pinned operation documents the standalone `apollo-mcp-server` binary serves, with `@oneOf` declared explicitly so the binary can parse the schema.

**Following along:** start from `mcp_class_3`, work through the lesson, then compare your result with this branch. The next class continues on `mcp_class_4b`.
