# graphQLMovieDB-MCP - Class 11

This branch is the repository state at the end of [Class 11: Elicitation and Sampling](https://graphqlguy.com/docs/tutorial-graphql-mcp/elicitation-and-sampling), part of the [GraphQL + MCP course](https://graphqlguy.com/docs/tutorial-graphql-mcp/mcp-and-graphql).

**What Class 11 adds:** `SamplingReviewSummarizer`, which asks the client's model for a summary, `ServerSideSummarizer` as the fallback when the client cannot, and the elicitation prompt behind `WatchStatusRequest`.

**Following along:** start from `mcp_class_10`, work through the lesson, then compare your result with this branch. The next class continues on `mcp_class_12`.
