...
Companion code for the [GraphQL + MCP course](https://graphqlguy.com/docs/tutorial-graphql-mcp/mcp-and-graphql).

**`main` is the course's starting point.** It holds the Movie Database application as the course expects it before any MCP work, and each class branch, `mcp_class_2` through `mcp_class_19` plus `mcp_class_4b`, adds that class's work on top. `mcp_class_19` is the finished state.

Start here and code along with the lessons, or switch to a class branch to see where a lesson ends up:
