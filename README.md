# graphQLMovieDB-MCP - Class 2

This branch is the repository state at the end of [Class 2: Extending the Movie Schema for Agents](https://graphqlguy.com/docs/tutorial-graphql-mcp/extending-schema-for-agents), part of the [GraphQL + MCP course](https://graphqlguy.com/docs/tutorial-graphql-mcp/mcp-and-graphql).

**What Class 2 adds:** the two agent-designed operations, `recommendMoviesForMood` (with the `Mood` enum and `MoodProfile` behind it) and `summarizeMovieReviews`, plus runtime filtering on `MovieFilter` and the frontend pages that use them.

**Following along:** start from `main`, work through the lesson, then compare your result with this branch. The next class continues on `mcp_class_3`.
