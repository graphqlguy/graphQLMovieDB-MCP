# graphQLMovieDB-MCP - Class 7

This branch is the repository state at the end of [Class 7: Translating GraphQL Operations Into MCP Tools](https://graphqlguy.com/docs/tutorial-graphql-mcp/translating-operations-to-tools), part of the [GraphQL + MCP course](https://graphqlguy.com/docs/tutorial-graphql-mcp/mcp-and-graphql).

**What Class 7 adds:** `MovieMcpTools`, the `@McpTool` methods an agent calls, and `MovieOperations`, the pinned GraphQL documents they run through `ExecutionGraphQlService`.

**Following along:** start from `mcp_class_6`, work through the lesson, then compare your result with this branch. The next class continues on `mcp_class_8`.
