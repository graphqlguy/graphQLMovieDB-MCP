# graphQLMovieDB-MCP - Class 15

This branch is the repository state at the end of [Class 15: OAuth 2.1 Resource Server](https://graphqlguy.com/docs/tutorial-graphql-mcp/oauth2-resource-server), part of the [GraphQL + MCP course](https://graphqlguy.com/docs/tutorial-graphql-mcp/mcp-and-graphql).

**What Class 15 adds:** `McpSecurityConfig`, which puts `/mcp` behind an OAuth 2.1 resource server, with the tests that prove an unauthenticated call is refused.

**Following along:** start from `mcp_class_14`, work through the lesson, then compare your result with this branch. The next class continues on `mcp_class_16`.
